...
describe("@noahark/custom-fields package boundary", () => {
      ...
      if (
        specs.some(
          (spec) =>
            ...
            spec.startsWith("@noahark/catalog") ||
  ...
  it("does not import another test file", () => {
    const tests = walkTsFiles(join(pkgRoot, "src")).filter((f) => f.endsWith(".test.ts"));
    for (const file of tests) {
      const text = readFileSync(file, "utf8");
      expect(text).not.toMatch(/from\s+["'][^"']+\.test(?:\.ts)?["']/);
    }
  });
});
